...
namespace X12.Sql
{
    ...
    internal class SegmentBatch
    {
        ...
        public void AddSegment(SqlTransaction tran, object interchangeId, int positionInInterchange, int revisionId, object functionalGroupId, object transactionSetId, object parentLoopId, object loopId, bool deleted, DetachedSegment segment, SegmentSpecification spec)
        {
            this.SegmentTable.Rows.Add(interchangeId, positionInInterchange, revisionId, functionalGroupId, transactionSetId, parentLoopId, loopId, deleted, segment.SegmentId, segment.SegmentString);
            if (spec != null)
            {
                ...
                for (var i = 1; i <= segment.ElementCount && i <= maxElements; i++)
                {
                    try
                    {
                        ...
                        {
                            var message = string.Format(Resources.ElementTruncatedWarning, interchangeId, positionInInterchange, segment.SegmentId, i, maxLength);
                        ...
                        if (elementSpec.Type == ElementDataType.Numeric && elementSpec.ImpliedDecimalPlaces > 0)
                        {
                            ...
                            {
                                var message = string.Format("Element {2}{3:00} in position {1} of interchange {0} cannot be indexed because '{4}' could not be parsed into an implied decimal with precision {5}.", interchangeId, positionInInterchange, segment.SegmentId, i, val, elementSpec.ImpliedDecimalPlaces);
                                Trace.TraceInformation(message);
                                parsingError.AppendLine(message);
                                row[column] = null;
                            }
                        ...
                        else if (elementSpec.Type == ElementDataType.Numeric || elementSpec.Type == ElementDataType.Decimal)
                        {
                            ...
                            {
                                var message = string.Format("Element {2}{3:00} in position {1} of interchange {0} cannot be indexed because '{4}' could not be parsed into a decimal.", interchangeId, positionInInterchange, segment.SegmentId, i, val);
                                Trace.TraceInformation(message);
                                parsingError.AppendLine(message);
                                row[column] = null;
                            }
                        ...
                        else if (elementSpec.Type == ElementDataType.Date)
                        {
                            ...
                            {
                                DateTime date;
                                if (val.Length == 8 && DateTime.TryParse($"{val.Substring(0, 4)}-{val.Substring(4, 2)}-{val.Substring(6, 2)}", out date))
                                {
                                    row[column] = date;
                                }
                                else
                                {
                                    var message = string.Format("Element {2}{3:00} in position {1} of interchange {0} cannot be indexed because '{4}' could not be parsed into a date.", interchangeId, positionInInterchange, segment.SegmentId, i, val);
                                    Trace.TraceInformation(message);
                                    parsingError.AppendLine(message);
                                    row[column] = null;
                                }
                    ...
                    {
                        var message = string.Format("Error parsing '{0}' using spec {1} with {2} elements: {3}", segment.SegmentString, spec.SegmentId, spec.Elements.Count(), e.Message);
                        Trace.TraceInformation(message);
                        parsingError.AppendLine(message);
                    }
                }
                ...
                {
                    row["ErrorId"] = this.errorRepo.PersistParsingError(interchangeId, positionInInterchange, revisionId, parsingError.ToString());
                }
        ...
        public void AddLoop(object id, Loop loop, object interchangeId, object transactionSetId, string transactionSetCode, object parentLoopId, string entityIdentifierCode)
        {
            var row = this.LoopTable.NewRow();
            row["Id"] = id;
            row["ParentLoopId"] = !(parentLoopId == null || parentLoopId == this.defaultIdentityTypeValue) ? parentLoopId : DBNull.Value;
            row["InterchangeId"] = interchangeId;
            row["TransactionSetId"] = !(transactionSetId == null || transactionSetId == this.defaultIdentityTypeValue) ? transactionSetId : DBNull.Value;
