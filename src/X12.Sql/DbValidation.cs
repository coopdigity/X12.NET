...
namespace X12.Sql
{
    ...
    public class DbValidation : IValidation
    {
        ...
        {
            var result = this.executor.ExecuteScalar(new SqlCommand($"SELECT CASE WHEN EXISTS (SELECT * FROM sys.schemas WHERE name = N'{this.Schema}') THEN 1 ELSE 0 END"));
        ...
        {
            var result = this.executor.ExecuteScalar(new SqlCommand(string.Format(@"SELECT CASE WHEN EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'[{0}].[{1}]') AND type IN (N'FN', N'IF', N'TF', N'FS', N'FT')) THEN 1 ELSE 0 END", this.Schema, functionName)));
            return Convert.ToInt32(result) != 0;
        }
        ...
        {
            var result = this.executor.ExecuteScalar(new SqlCommand($"SELECT CASE WHEN EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'[{this.Schema}].[{tableName}]') AND type IN (N'U')) THEN 1 ELSE 0 END"));
            return Convert.ToInt32(result) != 0;
        }
        ...
        {
            var result = this.executor.ExecuteScalar(new SqlCommand($"SELECT CASE WHEN EXISTS (SELECT * FROM sys.views WHERE object_id = OBJECT_ID(N'[{this.Schema}].[{viewName}]')) THEN 1 ELSE 0 END"));
            return Convert.ToInt32(result) != 0;
        }
        ...
        {
            var result = this.executor.ExecuteScalar(new SqlCommand(string.Format(@"SELECT CASE WHEN EXISTS 
(SELECT *
