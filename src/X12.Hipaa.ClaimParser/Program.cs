namespace X12.Hipaa.ClaimParser
{
    ...
    using System.Linq;
    using System.Xml;
    //using Fonet; // unmaintained PDF engine
    ...
    public class Program
    {
        ...
        public static void Main(string[] args)
        {
            ...
            var service = new ClaimFormTransformationService(new ProfessionalClaimToHcfa1500FormTransformation("HCFA1500_Red.gif"), institutionalClaimToUb04ClaimFormTransformation, new DentalClaimToJ400FormTransformation("ADAJ400_Red.gif"), new X12Parser(throwException));
            foreach (var filename in Directory.GetFiles(opts.Path, opts.SearchPattern, SearchOption.TopDirectoryOnly))
            {
                try
                {
                    ...
                    var revenueDictionary = new Dictionary<string, string>{["0572"] = "Test Code"};
                    ...
                    if (opts.MakePdf)
                    {
                        throw new Exception("Make PDF option unsupported");
                        /* FoNet unsupported in .net core
